...
public class AutoCar2Controller : Singleton<AutoCar2Controller>
{
    ...
    private void Start()
    {
        ...
        typeCar = GameManager.Instant.GetIdAutoCar(1);
        InitCar();
    }
    ...
    {
        targetPosition = new Vector3(-22, transform.position.y, transform.position.z);
        car.Move(3, transform.position, targetPosition);
    }
    public void MoveDestination()
    {
        targetPosition = new Vector3(19, transform.position.y, transform.position.z);
        car.Move(1.5f, transform.position, targetPosition);
    }
}
